...
"#include \"arch.hpp\"\n";
	...
	unsigned count = grp_root->get_count();
	for (unsigned ii=0; ii<count; ii++) {
		ofile << "void impl_" << grp_root->get_qualified_name(ii)
...
void isa_prog::emit_codegenerator(ofstream& ofile)
{
	...
	for (unsigned ii=0; ii<count; ii++) {
		ofile << "char *emitc_" << grp_root->get_qualified_name(ii)
			<< "(char *buf, emulator_t *emu, target_addr_t curpc, "
			<< "target_addr_t fstart, target_addr_t fend)\n"
			<< "{\n";
